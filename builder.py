...
class QuantKosBuilder:
    ...
    @todo Should add the possibility of activate tests and release mode
    """
    ...
    def __init__(self,
                 ...
                 yaml_cpp_path: Optional[str] = None) -> None:
        """!
    ...
    def from_interactive(cls) -> "QuantKosBuilder":
        ...
        while True:
            ...
            try:
                # 1. QuantKos Root Directory (must exist)
                ...
                default_workspace = os.path.abspath(os.path.join(src_root, '..', 'TempBuilds&Installs'))
                ...
                if not use_fetchcontent:
                    ...
                    yaml_cpp_path = cls._get_valid_cmake_config(
                        prompt_text="\n> yaml-cpp CMake Config Path",
                    ...
                    default_kokkos_paths_mapping = {
                        cls.ImplementedBackends.SERIAL.name: os.path.join(default_install, 'kokkos-5', 'single_core', 'lib', 'cmake', 'Kokkos'),
                        cls.ImplementedBackends.OPENMP.name: os.path.join(default_install, 'kokkos-5', 'openmp', 'lib', 'cmake', 'Kokkos'),
                        cls.ImplementedBackends.CUDA.name: os.path.join(default_install, 'kokkos-5', 'cuda', 'lib', 'cmake', 'Kokkos'),
                    }
                    ...
                    for backend in cls.ImplementedBackends:
                        ...
                        kokkos_paths[backend_name] = cls._get_valid_cmake_config(
                            prompt_text=f"\n> Kokkos [{backend_name}] Path",
                            default_path=default_p
                        )

                # --- 5. Summary & Final Confirmation Step ---
                print("\n\n" + "*" * 60)
    ...
    @staticmethod
    def _get_valid_cmake_config(prompt_text: str, default_path: str) -> str:
        """!
    ...
    def build_and_install(self,
        ...
        target_suffix = f"{backend.lower()}_{precision.lower()}_{math.lower()}"
        ...
        # --- FETCHCONTENT (AUTOMATIC DOWNLOADS) ---
            ...
            if backend == self.ImplementedBackends.SERIAL.name:
                ...
                flags.extend([
                    ...
                    "-DKokkos_ARCH_AMPERE86=ON"  # should address this with an enumerate
                ])
            else:
